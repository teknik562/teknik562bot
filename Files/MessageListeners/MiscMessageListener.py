...
class MiscMessageListener(IMessageListener):
    ...
    def handle_getfile(update: Update):
        ...
        TelegramBotManager().bot.send_document(chat_id=update.message.chat_id, document=func_result, timeout=300)

    @staticmethod
    def handle_kamera(update: Update):
        telegram_id = BotUtil.get_telegram_id(update)
    ...
    def handle_alarmdegistir(update: Update):
        ...
        if MiscMessageListener.alarm_state == True:
            ...
            TelegramBotManager().bot.send_message(chat_id=update.message.chat_id, text=f'Alarm Durumu: KAPALI')
        else:
            MiscMessageListener.alarm_state = True
            TelegramBotManager().bot.send_message(chat_id=update.message.chat_id, text=f'Alarm Durumu: ACIK')
        
        #TelegramBotManager().bot.send_message(chat_id=update.message.chat_id, text=f'Alarm Durumu: {MiscMessageListener.alarm_state}')
            
    @staticmethod
    def get_alarm_state():
        global alarm_state
        return MiscMessageListener.alarm_state    
