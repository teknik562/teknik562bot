    ...
    def capture_video(video_length):
        try:
            if os.path.isfile('out.avi'):
                os.remove('out.avi')
            command = f'ffmpeg -s 640x480 -t {video_length} -r 18 -i /dev/video0 out.avi'
            #command = f'ffmpeg -s 640x480 -t {video_length} -r 30 -i /dev/video0 out.avi'
            #command = f'ffmpeg -f v4l2 -t {video_length} -i /dev/video0 out.avi -r 60 -profile:v high -pix_fmt yuvj420p -level:v 4.1 -tune zerolatency -vcodec libx264 -b:v 512k -s 640x480' 
            #command = f'ffmpeg -f video4linux2 -r 12 -s 640x480 -t 5 -i /dev/video0 out.avi'
